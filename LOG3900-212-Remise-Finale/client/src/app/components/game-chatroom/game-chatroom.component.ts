...
@Component({
    selector: 'app-game-chatroom',
    standalone: true,
    imports: [CommonModule, FormsModule, TranslateModule],
    templateUrl: './game-chatroom.component.html',
    styleUrls: ['./game-chatroom.component.scss'],
})
export class GameChatroomComponent implements OnInit, OnDestroy {
    ...
    sendMessage(): void {
        ...
        const hasMedia = this.previewFiles.length > 0;
        ...
        if (hasValidText) {
            ...
            this.messageText = '';
            this.socketService.sendMessage(ChatEvents.Message, { channelId: this.gameId, message: textMessage });
        }
        ...
        if (hasMedia) {
            this.previewFiles.forEach((preview) => {
                ...
                this.socketService.sendMessage(ChatEvents.Message, { channelId: this.gameId, message: newMessage });
            });
        }
        this.messageText = '';
        this.previewFiles = [];
                ...
                for (const file of Array.from(files)) {
                    if (!file.type.startsWith('video/')) {
                        ...
                        if (file.size > MAX_FILE_SZIE) {
                            console.log(`Video ${file.name} is too large. Maximum size is 10MB.`);
                            continue;
                        }
    ...
    getUserAvatar(avatar: Avatar): string {
        return this.avatars.find((a) => a.id === avatar)?.preview || '';
    }
